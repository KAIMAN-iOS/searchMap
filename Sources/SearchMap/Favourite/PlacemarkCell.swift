...
class PlacemarkCell: UITableViewCell {
    ...
    func configure(_ favType: FavouriteType, place placemark: Placemark?) {
        ...
        name.set(text: placemark?.name ?? favType.name, for: FontType.default, textColor: placemark == nil ? FavouriteListViewController.configuration.palette.inactive : FavouriteListViewController.configuration.palette.mainTexts)
    ...
    func configure(_ placemark: Placemark, iconTintColor: UIColor = FavouriteListViewController.configuration.palette.primary) {
        ...
        name.set(text: placemark.name, for: FontType.default, textColor: FavouriteListViewController.configuration.palette.mainTexts)
        if placemark.address?.isEmpty == false {
            address.isHidden = false
            address.set(text: placemark.address, for: FontType.footnote, textColor: FavouriteListViewController.configuration.palette.inactive)
        } else {
            address.isHidden = true
        }
    }
